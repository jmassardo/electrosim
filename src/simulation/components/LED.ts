...
export class LEDComponent extends DigitalComponent {
  ...
  public update(_context: ComponentUpdateContext): void {
    ...
    if (this.isOn) {
      ...
      const current = Math.min(this.maxCurrent, excessVoltage * 0.02); // Increased multiplier for more realistic current
      
      // For PWM simulation, brightness should be proportional to the applied voltage relative to maximum supply
      // This gives more intuitive brightness behavior matching PWM duty cycles
      const maxVoltage = 5.0; // Assume 5V supply
      this.brightness = Math.min(1.0, voltageDrop / maxVoltage);
    ...
    if (wasOn !== this.isOn) {
      console.log(`LED ${this.id} ${this.isOn ? 'turned ON' : 'turned OFF'} (V=${voltageDrop.toFixed(2)}V, I=${(this.getPinCurrent('Anode') * 1000).toFixed(1)}mA, brightness=${this.brightness.toFixed(2)})`);
    }
  }
  ...
  public validate(): string[] {
    ...
    const anodePin = this.pins.get('Anode');
    const cathodePin = this.pins.get('Cathode');
    
    if (!anodePin?.connected) {
      errors.push('Anode pin is not connected');
    }
    
    if (!cathodePin?.connected) {
      errors.push('Cathode pin is not connected');
    }
